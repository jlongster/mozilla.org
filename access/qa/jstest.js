...
function setupListeners(iframe)
{
  ...
  iframe.contentDocument.addEventListener("keyup",testKeyUp,false); 
};

function testDOMNode(domNode, x, y, useCaret)
{
  var accService = null;
  gAccNode = null;
  ...
  updateDisplay(domNode, x, y, useCaret);
};
...
function updateDisplay(domNode, x, y, useCaret)
{
  ...
  if (gAccText) {
    ...
    if (useCaret) {
      // Use caret
      offsetAtPoint = gAccNode.caretOffset;
    }
    else if (x > 0 || y > 0) {
      offsetAtPoint = gAccNode.getOffsetAtPoint(x, y, Components.interfaces.nsIAccessibleText.COORD_TYPE_SCREEN);
    }
    var extentX, extentY, extentWidth, extentHeight;
    //gAccNode.getCharacterExtents(offsetAtPoint, extentX, extentY, extentWidth, extentHeight, Components.interfaces.nsIAccessibleText.COORD_TYPE_SCREEN);
    document.getElementById("offsetAtPoint").firstChild.nodeValue = offsetAtPoint; // + " [Bounds = " + extentX.value +","+ extentY.value+","+ extentWidth.value+","+extentHeight.value + "]";
    characterCount = gAccNode.characterCount;
    var startOfLineOffset = offsetAtPoint; // characterCount;
    try { document.getElementById("name").firstChild.nodeValue = gAccNode.name; } catch(ex) {}
    var before = { }, after = { };
    text = "";
    var count = 0;
...
{
  if (!document.getElementById("track-keyup").checked)
    return;  // Test clicks option is off
  if (targetInTestUI(evt.target)) {
    return;
  }

  var contentWindow = gLastFocus.ownerDocument ? gLastFocus.ownerDocument.defaultView : gLastFocus.defaultView;
  ...
  testDOMNode(startContainer, 0, 0, true);
};
